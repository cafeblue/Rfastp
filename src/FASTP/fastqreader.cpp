...
Read* FastqReader::read(){
    ...
    if (!mHasQuality){
        ...
        if(quality.length() != sequence.length()) {
            Rcpp::Rcerr << "ERROR: sequence and quality have different length:" << endl;
            Rcpp::Rcerr << name << endl;
            Rcpp::Rcerr << sequence << endl;
            Rcpp::Rcerr << strand << endl;
            Rcpp::Rcerr << quality << endl;
            return NULL;
        }
        return new Read(name, sequence, strand, quality, mPhred64);
    }
